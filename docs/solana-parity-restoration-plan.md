...
| 2026-08-27 | 3           | ✅ Folded into Phase 2. `packages/abis/src/chain.ts` + `solana-addresses.ts` restored & built; `@loar/abis` typecheck PASS. `addresses.ts` needs **no** Solana reconciliation — its snapshot→main drift is purely the Base-Sepolia (`84532`) removal, which parity restoration correctly leaves in place; all Solana program/mint/tree addresses live in `solana-addresses.ts`. `apps/indexer` (Ponder, EVM) untouched, typecheck PASS.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 2026-08-27 | 4a          | ✅ Web restore. Restored `components/{SolanaPayButton,SolanaMintDialog}.tsx`, `hooks/useCircleSolanaAddress.ts`, `routes/{solana,bridge}.tsx` from `4cff84fb`; regenerated `routeTree.gen.ts` (adds `/solana` + `/bridge`, 0 deletions). `qrcode.react` already present; **no `@solana/wallet-adapter` needed** — web architecture is Circle-DCW custodial + Solana Pay QR (browser wallet is mobile-only). All 5 files call Phase-2 server routes. **`tsc --noEmit` PASS · web unit tests 36/36.** Did not touch the concurrently-edited `useUniverseBlockchain.ts` / `universe/$id.tsx`.                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 2026-08-27 | 4b          | ✅ Multi-chain UI seam (net-new). `configs/chains.ts`: widened `ChainSelection` to `{kind:'evm'} \| {kind:'solana',cluster}`, added `solanaOption`/`SOLANA_ENABLED`/`ACTIVE_SOLANA_CLUSTER` (gated on `VITE_SOLANA_CLUSTER` — Solana appears in the picker only once wired), + `selectionIdToSelection`/`isEvmSelection`/`isSolanaSelection`/`selectionNamespace` helpers. New `hooks/useChain.ts` — `useSyncExternalStore` store, localStorage-persisted, cross-tab, exposes `{chain,namespace,isEvm,isSolana,evmChainId,solanaCluster,setChain,setChainById}`. New `lib/governance.ts` — `useGovernance()` seam: EVM → pass-through to `useUniverseGovernor`; Solana → typed `disabled` descriptor (Realms wiring is the follow-up, no `/api/governance/*` Solana routes yet). Fixed `cinematicUniverseCreate.tsx` for the widened union (`.kind==='evm'` guards). **web `tsc --noEmit` PASS · unit 36/36.** Not adopted by the 2 existing local pickers yet (concurrent edits in that tree) — seam is in place for incremental migration. |
| 2026-08-27 | 5           | ✅ Mobile. Restored `apps/mobile/src/lib/solana-auth.ts` (Android MWA SIWS). Deps (`@solana-mobile/mobile-wallet-adapter-protocol-web3js`, `@solana/web3.js`, `bs58`, `buffer`) were never removed. `check-types` PASS. UI wiring still pending per `apps/mobile/SOLANA.md` (unchanged from prototype).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 2026-08-27 | 6           | ✅ CI + docs. `.github/workflows/{anchor-tests,bridge-reconcile}.yml` were **never deleted** — orphaned, now live again with `apps/programs/` restored (path-gated). Root `turbo check-types` auto-covers `@loar/solana-indexer`. Flipped the STATUS banner in all 5 Solana docs (`solana-overview`, `prd-solana-parity`, `prd-solana-native-sdk-glue`, `solana-bridge`, `solana-mainnet-runbook`) from "PLANNED / NOT YET ACTIVE" → "RESTORATION IN PROGRESS", pointing here; original text kept as historical context. `docs/solana-parity-matrix.md` regen deferred (derived doc, non-blocking).                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 2026-08-27 | 7 (partial) | ⏳ Devnet redeploy needs credentials not available here (funded devnet keypair, real program **upgrade keypairs**, Helius key, running server) — remains open. Done locally: `apps/solana-indexer` decoder smoke PASS; `anchor test` on a local validator **compiled + deployed 9–10/15 programs successfully** (no ID/ABI errors) then hit the default provider-wallet balance ceiling — an env funding limit, not a code defect. **CI fix landed** in `anchor-tests.yml`: mint throwaway keypairs for all 15 members + `anchor keys sync` (runner-only, never committed — canonical `declare_id!` IDs stay in git), and run against a self-started `solana-test-validator --mint <deployer>` (500M SOL) via `--skip-local-validator`. Remaining Phase 7: recover/rotate the real upgrade keypairs, `anchor deploy` to devnet, re-register the Helius webhook, run `mainnet-runbook-dryrun.ts` + `bridge-roundtrip-e2e.ts`.                                                                                                                 |
| 2026-08-28 | auth        | ✅ Dual-wallet at sign-up. `circle-auth.ts` `/verify-otp` + `/social` now eagerly provision the Circle DCW **Solana** wallet alongside the EVM one via `provisionSolanaWalletAtSignup()` — gated on `isCircleSolanaConfigured()`, non-fatal (Circle outage ⇒ login still succeeds, no `sol` claim, lazy path retries). Keyed by the lowercased EVM address so it's the same wallet `GET /api/solana/wallet` resolves; idempotent, so it also backfills existing users on next login. JWT gains the `sol` claim; response body gains `solanaAddress`. +3 `circle-auth.test.ts` cases (provisioned / not-configured / provisioning-throws). **server tsc PASS · suite 238 pass / 42 skip** (same 3 emulator-only failures).                                                                                                                                                                                                                                                                                                                    |
