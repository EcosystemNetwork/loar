...
| Episode NFT mint loop                | List → discover → mint with ETH on Ethereum Sepolia. MintContentDialog, BuyNFTDialog, batch listing, ERC2981 royalty routing all wired                                                                                                                                                                                                                                                     |
...
| Solana parity (planned future chain) | Prototyped on devnet, not live — archived in branch `archive/solana-base-support` (tag `solana-base-snapshot`). Prototype covered cNFT mints, canon promotion, $LOAR Token-2022 with mint locked, custodial bridge with per-tx + per-day caps, indexer healthz, Squads handoff scripts. Restore tracked for a future release                                                               |
...
| #   | ID                          | Action                                                                                                                                                                | Unblocker                                                                                                                     |
| --- | --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| O1  | DMCA-01 op                  | Set `DMCA_PUTBACK_ENABLED=true` on exactly one prod replica                                                                                                           | env var on Railway                                                                                                            |
| O2  | GOV-01                      | Deploy Safe (3/5), TimelockController (48h), run `script/TransferToMultisig.s.sol` on Ethereum Mainnet                                                                | Pick signers, fund deployer, run script                                                                                       |
...
> **Note:** Rows O7–O13 are Solana-specific (`SOL-*`). Solana is a **planned future chain** (prototyped on devnet, archived in branch `archive/solana-base-support`), not part of the current Ethereum Mainnet critical path. They are retained here as the future-chain restore backlog.
...
| X7  | SOL-AUDIT-01      | _(Planned future chain — Solana, archived)_ External audit of Anchor programs (universe / episode / payment) — shortlist + outreach template in [external-audit-engagement.md](./external-audit-engagement.md) | Audit firm                                         | 2–4 wk lead, 3–6 wk audit |
...
| X13 | SOL-NTT-01        | _(Planned future chain — Solana, archived)_ Migrate custodial bridge to Wormhole NTT (deploy NTT manager + transceiver on Solana + Ethereum)                                                                   | Wormhole integration                               | 2–4 wk                    |
...
DRY_RUN=true forge script script/TransferToMultisig.s.sol --rpc-url <ethereum-mainnet>
```
...
| [solana-mainnet-runbook.md](solana-mainnet-runbook.md) | _Planned future chain (archived):_ devnet → mainnet Solana ops, multisig handoff steps |
| [solana-overview.md](solana-overview.md)               | _Planned future chain (archived):_ Solana architecture umbrella                        |
