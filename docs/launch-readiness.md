...
**Verdict:** Testnet beta live on Sepolia + Base Sepolia. Mainnet blocked on legal (3 items), operational handoffs (≥11 items), and external audit (1 pass).
...
| Contract security     | 130 / 157 audit findings fixed across 8 review passes (P0 14/15, P1 27/31, P2 24/25). Remaining items below                                          |
...
| X5  | EVM Pass 1        | Engage external audit firm on EVM contracts (NFT upgradeable rewrite, revenue routing, RIGHTS-01 sig path, UNIVERSE-01 are large new surfaces) | Audit firm                                         | 8–12 wk                   |
| X6  | EVM Pass 2        | Re-audit after Pass 1 fixes applied                                                                                                            | Audit firm                                         | 4–6 wk                    |
| X7  | SOL-AUDIT-01      | External audit of Anchor programs (universe / episode / payment) — OtterSec / Neodyme / Sec3 / Halborn                                         | Audit firm                                         | 2–4 wk lead, 3–6 wk audit |
...
| #   | ID                   | Action                                                                                                                               | Effort                         |
| --- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------ |
| C1  | BURN-01              | Update tokenomics docs to stop claiming deflationary burns; plan `LoarBurner` → `PremiumActions` rename for post-launch UUPS upgrade | <1 day docs                    |
| C2  | UPGRADE-01 follow-on | Commit baseline storage-layout JSON artifacts so future PRs can diff layouts automatically                                           | <1 day, after one clean CI run |
| C3  | UNIVERSE-02          | Decide: keep documented O(1) trade-off (off-chain consumers use `getCanonChain()`) or do walk-and-update refactor                    | Design call                    |
| C4  | ESCROW-03            | `Escrow.resolveDispute` DAO appeal path (still `onlyOwner`; mitigated by Timelock+Safe post-GOV-01)                                  | Design + 1–2 wk if pursued     |

---

## Deferred to post-launch (acknowledged, not blocking)

- **AD-02** — AdPlacement oracle / episode-mint proof for impression integrity
