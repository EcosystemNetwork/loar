...
- **Auth:** Circle Developer-Controlled Wallet (same SIWE-style JWT flow as web — `useAuth` / `useWalletAuth` hooks adapted for React Native). Email / google / apple / passkey sign-in; the wallet is provisioned and signed server-side, nothing custodial lives on the device.
...
| ------------------------------------------------------------------------------------ | ------------------------- | --------------------------------------------------------------------- |
| Circle Web3 Services SDK (for DCW provisioning + KMS-backed signing)                 | Circle / external         | Auth flow, all wallet transactions                                    |
