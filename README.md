...
| **Mobile App**            | Expo 52 / React Native (iOS + Android). Portfolio, tokens, earnings, collections, profile tabs. Circle DCW auth (email/social/passkey via the server-side flow shared with web). Production Hermes bytecode builds end-to-end (~16.6MB `.hbc`, 7083 modules). `@sentry/react-native` scaffold initialized in `app/_layout.tsx` (JS-layer crashes; native capture requires `expo prebuild`)  | Beta — not yet published to App Store / Play Store                                  |
...
| [Model Pricing](docs/pricing.md)                                                     | Per-model USD cost, fiat / LOAR price, credits — auto-generated (`pnpm docs:pricing`). Daily drift alerts (`pnpm cost:check-drift`), weekly staleness audit (`pnpm registry:check-staleness`), monthly invoice reconciliation (`pnpm reconcile:spend`). |
| [Pricing Integrity — Ops Runbook](docs/ops-pricing-integrity.md)                     | Credential setup, IAM roles, alert playbooks, secret rotation. Run `pnpm ops:firebase-doctor` when ops scripts fail.                                                                                                                                    |
