...
function isRegisteredClient(clientId, redirectUri) {
    ...
    return OAUTH_CLIENT_ALLOWLIST.some((e) => e.clientId === clientId && redirectUri.startsWith(e.redirectUriPrefix || ''));
}
    ...
    try {
        ...
        try {
            ...
            if (parsed?.method === 'tools/call')
                auditedTool = parsed?.params?.name;
        }
        catch {
        ...
        await entry.transport.handlePostMessage(replay, res);
        auditLog({
            kind: 'mcp.dispatch',
            sessionId,
