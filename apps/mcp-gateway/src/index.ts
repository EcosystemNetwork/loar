...
const transports = new Map<
  string,
  { transport: SSEServerTransport; walletAddress: string; close: () => Promise<void> }
>();
...
async function handleAuthorize(req: http.IncomingMessage, res: http.ServerResponse, url: URL) {
...
async function handleToken(req: http.IncomingMessage, res: http.ServerResponse) {
  ...
  if (!code || !codeVerifier || !redirectUri || !clientId) {
    return oauthError(
      res,
      'invalid_request',
      'missing code, code_verifier, redirect_uri, or client_id'
    );
  }
 ...
 */
async function mintKeyForWallet(walletAddress: string): Promise<string> {
  ...
  const json = (await res.json()) as { result?: { data?: { rawKey: string } } };
  const rawKey = json.result?.data?.rawKey;
  if (!rawKey) throw new Error('key-mint returned no rawKey');
...
async function handleSse(req: http.IncomingMessage, res: http.ServerResponse) {
  ...
  const close = async () => {
    if (!transports.delete(transport.sessionId)) return;
    try {
    ...
    console.error(
      `[gateway] session closed: ${transport.sessionId} wallet=${walletAddress} active=${transports.size}`
    );
  };

  transports.set(transport.sessionId, { transport, walletAddress, close });
  console.error(
    `[gateway] session opened: ${transport.sessionId} wallet=${walletAddress} active=${transports.size}`
  );
  req.on('close', close);
...
async function handleMessages(req: http.IncomingMessage, res: http.ServerResponse, url: URL) {
...
function oauthError(res: http.ServerResponse, code: string, description: string) {
  res
    .writeHead(400, { 'Content-Type': 'application/json' })
