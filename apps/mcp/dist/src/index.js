...
const JOB_KINDS = ['video', 'image', 'voice', '3d', 'studio'];
 ...
 */
async function pollAndStream(server, client, jobId, progressToken, signal) {
    ...
    while (Date.now() - start < MAX_POLL_DURATION_MS) {
        if (signal.aborted)
            return lastJob;
        try {
            const job = await client.query('jobs.status', { jobId });
            lastJob = job;
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
        ...
        try {
            ...
            if (progressToken !== undefined && jobRef && !isTerminalResult(result)) {
                const finalJob = await pollAndStream(server, client, jobRef.jobId, progressToken, extra.signal);
