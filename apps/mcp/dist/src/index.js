...
function hasPermission(required, actual) {
...
function checkRateLimit(toolName) {
    ...
    if (requiredLevel === 'read')
        return true; // no rate limit on reads
    const now = Date.now();
    // Prune old entries
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
    const permissionLevel = LOAR_MCP_PERMISSION_LEVEL;
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    const permissionLevel = LOAR_MCP_PERMISSION_LEVEL;
    if (!hasPermission(requiredLevel, permissionLevel)) {
        return {
            content: [
                {
                    type: 'text',
    ...
    if (sizeError) {
        return {
            content: [{ type: 'text', text: `Input validation failed: ${sizeError}` }],
            isError: true,
        };
    }
    ...
    for (const [key, value] of Object.entries(inputArgs)) {
        if ((key === 'universeAddress' || key === 'address' || key === 'walletAddress') &&
            ...
            return {
                content: [
                    { type: 'text', text: `Invalid Ethereum address for "${key}": ${value}` },
                ],
                isError: true,
            };
        }
