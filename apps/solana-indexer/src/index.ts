...
function verifyHeliusAuth(authHeader: string | undefined): boolean {
...
/**
 * Helius enhanced webhook surfaces Bubblegum compressed-NFT events under
 * `events.compressed[]`. The shape per Helius docs:
 *   { type: 'COMPRESSED_NFT_MINT', assetId, tree, leafIndex, newLeafOwner, ... }
 *
 * We extract these fields into top-level columns so downstream queries
 * (assetId → episode, by tree, etc.) don't need to scan blob payloads.
 */
interface HeliusCompressedEvent {
  type?: string;
  assetId?: string;
  tree?: string;
  leafIndex?: number;
...
function extractMintFromCompressed(events: unknown[] | undefined): HeliusCompressedEvent | null {
  if (!events) return null;
  for (const raw of events) {
    const ev = raw as HeliusCompressedEvent;
    if (ev?.type === 'COMPRESSED_NFT_MINT' && ev.assetId) return ev;
  }
  // Some Helius payloads send the mint without an explicit `type` — fall back
  // to any entry that has an assetId.
  for (const raw of events) {
    const ev = raw as HeliusCompressedEvent;
    if (ev?.assetId) return ev;
  }
  return null;
}
