// Top-level-await script. The `export {}` marker tells tsc to treat this as a
// module so top-level await is allowed.
export {};
...
const CLUSTER = (process.env.SOLANA_CLUSTER as 'devnet' | 'mainnet-beta' | undefined) ?? 'devnet';
...
async function preflight() {
  ...
  if (probe.status === 401) {
    throw new Error(
      `indexer rejected auth header (401) — HELIUS_WEBHOOK_SECRET mismatch between local + deployed`
    );
  }
  if (probe.status >= 500) {
...
async function listExisting(): Promise<HeliusWebhook[]> {
  const resp = await fetch(HELIUS_BASE);
  if (!resp.ok) throw new Error(`Helius list failed: ${resp.status} ${await resp.text()}`);
  return (await resp.json()) as HeliusWebhook[];
}
...
async function upsert() {
  ...
  if (match) {
    ...
    const resp = await fetch(
      `https://api.helius.xyz/v0/webhooks/${match.webhookID}?api-key=${HELIUS_API_KEY}`,
      {
        method: 'PUT',
    ...
    const created = (await resp.json()) as HeliusWebhook;
