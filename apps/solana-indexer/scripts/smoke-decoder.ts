...
if (!universe) fail('universe program not registered');
...
if (!decoded) fail('decoder returned null');
if (decoded.kind !== 'UniverseCreated') fail(`expected kind UniverseCreated, got ${decoded.kind}`);
...
if (!privateDecoded || privateDecoded.kind !== 'UniverseCreated') fail('private path failed');
...
if (unknownProgram !== null) fail('expected null for unknown programId');
...
if (!rights) fail('rights program not registered');
...
if (!rightsDecoded) fail('rights decoder returned null');
...
console.log(
  'OK: decoder smoke passed (UniverseCreated typed + RightsSetViaAttestation generic + unknown-disc + unknown-program)'
);
