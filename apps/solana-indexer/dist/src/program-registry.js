...
function registerProgram(name, envVar) {
    ...
    if (!programId) {
        throw new Error(`Program "${name}" has no programId — set ${envVar} or include "address" in IDL`);
    }
...
export const PROGRAM_BY_ID = new Map(PROGRAMS.map((p) => [p.programId, p]));
export const KNOWN_PROGRAM_IDS = new Set(PROGRAMS.map((p) => p.programId));
export function findProgram(programId) {
