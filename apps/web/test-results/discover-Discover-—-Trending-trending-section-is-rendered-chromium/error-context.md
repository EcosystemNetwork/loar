...
- Name: discover.spec.ts >> Discover — Trending >> trending section is rendered
- Location: e2e/discover.spec.ts:135:3
...
Error: strict mode violation: getByText(/trending/i).first().or(locator('body')) resolved to 2 elements:
    ...
    2) <div class="hidden md:flex items-center gap-2 text-sm text-muted-foreground">…</div> aka getByText('Trending Now')
