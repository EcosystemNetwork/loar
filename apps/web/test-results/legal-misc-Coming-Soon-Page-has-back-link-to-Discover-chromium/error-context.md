...
- Name: legal-misc.spec.ts >> Coming Soon Page >> has back link to Discover
- Location: e2e/legal-misc.spec.ts:96:3
...
Locator: getByRole('link', { name: /back to discover/i }).or(locator('a[href*="/discover"]').first())
Expected: visible
Error: strict mode violation: getByRole('link', { name: /back to discover/i }).or(locator('a[href*="/discover"]').first()) resolved to 2 elements:
...
```ts
  1   | /**
  2   |  * Legal Pages & Miscellaneous Routes Tests.
  3   |  *
  4   |  * Verifies:
  5   |  * - Terms of Service page loads with content
  6   |  * - Privacy Policy page loads with content
  7   |  * - Coming Soon page renders with message and back link
  8   |  * - Activity feed page loads
  9   |  * - Videos page loads
