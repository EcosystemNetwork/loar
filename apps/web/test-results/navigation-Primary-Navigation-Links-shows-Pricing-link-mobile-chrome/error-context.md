...
- Name: navigation.spec.ts >> Primary Navigation Links >> shows Pricing link
- Location: e2e/navigation.spec.ts:59:3
...
Locator:  locator('header nav').first().getByText('Pricing')
Expected: visible
Received: hidden
Timeout:  10000ms
...
Call log:
  ...
  - waiting for locator('header nav').first().getByText('Pricing')
    13 × locator resolved to <a href="/pricing" class="px-2.5 py-1.5 rounded-md text-sm font-medium transition-colors whitespace-nowrap text-muted-foreground hover:text-foreground hover:bg-muted/50">Pricing</a>
