...
- Name: discover.spec.ts >> Discover — Tab Navigation >> Creators tab is visible
- Location: e2e/discover.spec.ts:40:3
...
Locator: getByRole('tab', { name: /creators/i }).or(getByText(/creators/i).first())
Expected: visible
Error: strict mode violation: getByRole('tab', { name: /creators/i }).or(getByText(/creators/i).first()) resolved to 2 elements:
