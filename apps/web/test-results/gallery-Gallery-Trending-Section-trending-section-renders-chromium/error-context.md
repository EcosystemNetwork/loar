...
- Name: gallery.spec.ts >> Gallery Trending Section >> trending section renders
- Location: e2e/gallery.spec.ts:87:3
...
Locator: getByText(/trending/i).first().or(locator('body'))
Expected: visible
Error: strict mode violation: getByText(/trending/i).first().or(locator('body')) resolved to 2 elements:
    1) <body>…</body> aka locator('body')
    2) <button data-slot="button" class="inline-flex items-center justify-center font-medium transition-all disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive hover:bg-accent hover:text-accent-foreground dark:hover:bg-accent/50 ro…>Trending</button> aka getByRole('button', { name: 'Trending' })

Call log:
  - Expect "toBeVisible" with timeout 10000ms
  - waiting for getByText(/trending/i).first().or(locator('body'))
...
```ts
  1   | /**
  2   |  * Gallery Tests — Content browsing, filtering, lightbox, and content cards.
  3   |  *
  4   |  * Verifies:
  5   |  * - Gallery page loads with heading and filters
  6   |  * - Media type filter buttons render (All, Video, Image, Audio, 3D)
  7   |  * - Sort options render (Newest, Trending, Price: Low, Price: High)
  8   |  * - Origin filter renders (All Origins, AI Generated, Uploaded)
  9   |  * - Search input is present
  10  |  * - Trending section renders
  11  |  * - Empty state message shows when no content
  12  |  * - Content cards display thumbnail, title, and metadata
  13  |  * - Clicking a filter updates displayed content
  14  |  */
  15  |
