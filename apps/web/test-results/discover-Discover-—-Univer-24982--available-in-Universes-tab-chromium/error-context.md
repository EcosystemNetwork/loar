...
- Name: discover.spec.ts >> Discover — Universes Tab Filters >> sort options are available in Universes tab
- Location: e2e/discover.spec.ts:110:3

# Error details

```
Error: locator.click: Error: strict mode violation: getByRole('tab', { name: /universes/i }).or(getByText(/universes/i).first()) resolved to 2 elements:
