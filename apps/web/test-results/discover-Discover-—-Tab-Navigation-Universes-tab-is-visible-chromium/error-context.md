...
- Name: discover.spec.ts >> Discover — Tab Navigation >> Universes tab is visible
- Location: e2e/discover.spec.ts:35:3
...
Locator: getByRole('tab', { name: /universes/i }).or(getByText(/universes/i).first())
Expected: visible
Error: strict mode violation: getByRole('tab', { name: /universes/i }).or(getByText(/universes/i).first()) resolved to 2 elements:
    ...
    2) <button role="tab" type="button" tabindex="-1" data-state="active" aria-selected="true" data-orientation="horizontal" data-radix-collection-item="" id="radix-:r2:-trigger-universes" aria-controls="radix-:r2:-content-universes" class="inline-flex items-center justify-center whitespace-nowrap rounded-sm px-3 py-1.5 text-sm font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disa…>…</button> aka getByRole('tab', { name: 'Universes' })

Call log:
  - Expect "toBeVisible" with timeout 10000ms
  - waiting for getByRole('tab', { name: /universes/i }).or(getByText(/universes/i).first())
