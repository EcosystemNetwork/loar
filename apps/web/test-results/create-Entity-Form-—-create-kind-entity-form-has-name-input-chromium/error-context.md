...
- Name: create.spec.ts >> Entity Form — /create/$kind >> entity form has name input
- Location: e2e/create.spec.ts:92:3
...
Locator: getByPlaceholder(/name/i).first().or(locator('input').first())
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for getByPlaceholder(/name/i).first().or(locator('input').first())
