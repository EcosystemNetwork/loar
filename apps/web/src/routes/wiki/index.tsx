...
function WikiPage() {
  ...
  const collections = useMemo(() => {
    const uniqueCollections: string[] = Array.from(
      new Set(characters.map((char: Character) => char.collection))
    );
  ...
  React.useEffect(() => {
    if (selectedCollection !== 'all') {
      ...
      const validTraits = new Set(
        Array.from(selectedTraits).filter((trait) => availableTraits.includes(trait))
      );
  ...
  const filteredCharacters = useMemo(() => {
    ...
    if (selectedCollection !== 'all') {
      filtered = filtered.filter(
        (character: Character) => character.collection === selectedCollection
      );
    }
    ...
    if (selectedTraits.size > 0) {
      filtered = filtered.filter((character: Character) => {
        const characterTraits = Object.entries(character.traits).map(
  ...
  const hasActiveFilters =
    searchQuery.trim() || selectedCollection !== 'all' || selectedTraits.size > 0;
                      ...
                      <>
                        ...
                        {availableTraits.length > 15 && (
                          <div className="p-2 text-xs text-muted-foreground border-t">
                            Showing first 15 of {availableTraits.length} traits for{' '}
                            {selectedCollection}
                          </div>
                        )}
                      </>
