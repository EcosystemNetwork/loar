/**
 * CreateLikenessFlow — Create a likeness entity + marketplace listing.
 *
 * Rendered by both /create/likeness and the "New listing" tab of
...
interface CreateLikenessFlowProps {
  /**
   * Embedded mode (inside My Listings): the page provides the heading, the
   * first-step Back button calls `onExit` instead of leaving for /create, and
   * the success screen's "My Listings" actions call `onDone` instead of
   * navigating to the page we're already on.
   */
  embedded?: boolean;
  onExit?: () => void;
  onDone?: () => void;
...
export function CreateLikenessFlow({ embedded, onExit, onDone }: CreateLikenessFlowProps = {}) {
  ...
  if (stage === 'success' && newListingId) {
    return (
      <div
        className={
          embedded ? 'py-10 text-center' : 'container mx-auto max-w-2xl px-4 py-16 text-center'
        }
      >
        ...
        <div className="flex gap-2 justify-center">
          {newListingPendingVerification ? (
            <Button
              onClick={() =>
                embedded ? onDone?.() : navigate({ to: '/marketplace/likeness/my-listings' })
              }
            >
              Verify identity
            </Button>
            ...
            <Button
              onClick={() => (embedded ? onDone?.() : navigate({ to: '/marketplace/likeness' }))}
            >
              {embedded ? 'See my listings' : 'Browse marketplace'}
            </Button>
          )}
  ...
  return (
    <div className={embedded ? '' : 'container mx-auto max-w-3xl px-4 py-8'}>
      {!embedded && (
        ...
        <div className="flex items-center justify-between mt-6">
          <Button
            ...
            onClick={() => {
              ...
              else if (embedded) onExit?.();
              else navigate({ to: '/create' });
            }}
          >
            <ChevronLeft className="size-4 mr-1" />
            {stage === 'upload' ? (embedded ? 'Cancel' : 'Back to /create') : 'Back'}
          </Button>
