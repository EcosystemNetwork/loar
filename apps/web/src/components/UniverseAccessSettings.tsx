...
export function UniverseAccessSettings({ universeId, onClose }: UniverseAccessSettingsProps) {
  ...
  const { data: currentAccessModel, isLoading: isLoadingAccess } = useQuery({
  ...
  // Sync tiers when loaded
  useEffect(() => {
    if (existingTiers && Array.isArray(existingTiers) && existingTiers.length > 0) {
      setTiers(
        existingTiers.map((t: any) => ({
          tier: t.tier || 'BASIC',
          pricePerMonth: String(t.pricePerMonth ?? '0'),
          earlyAccess: t.earlyAccess ?? false,
          votingBoost: t.votingBoost ?? false,
          premiumContent: t.premiumContent ?? false,
          behindTheScenes: t.behindTheScenes ?? false,
          creditBonus: t.creditBonus ?? 0,
        }))
      );
    }
  }, [existingTiers]);
    ...
    try {
      ...
      if (accessModel === 'subscription' || accessModel === 'both') {
        ...
        for (const tier of tiers) {
          try {
            await trpcClient.subscriptions.configureTier.mutate({
              universeId,
            ...
            tierErrors.push(`${tier.tier}: ${err instanceof Error ? err.message : 'failed'}`);
          }
        }
        if (tierErrors.length > 0) {
          toast.error(`Some tiers failed to save: ${tierErrors.join(', ')}`);
        }
      }
  ...
  const isLoading = isLoadingAccess || isLoadingTiers;
                    ...
                    <CardContent className="p-4 space-y-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            updateTier(i, 'creditBonus', parseInt(e.target.value) || 0)
                          }
