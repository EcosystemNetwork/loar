...
export function UniverseSidebar({
  ...
  return (
    <>
      ...
      {/* Desktop layout slot. Only the pinned sidebar takes real width: the
          collapsed rail hover-expands as an overlay on top of the canvas, so
          opening it doesn't resize the canvas and shove the toolbar around. */}
      <div
        className={`md:relative md:h-full md:shrink-0 transition-[width] duration-300 ease-in-out ${pinned ? 'md:w-80' : 'md:w-16'}`}
      >
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      queryClient.invalidateQueries({
                        queryKey: ['universe-metadata', universeId],
                      });
