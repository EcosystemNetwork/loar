  ...
  const generateVideoMutation = useMutation({
    mutationFn: async ({ imageUrl, prompt }: { imageUrl: string; prompt?: string }) => {
      ...
      if (selectedVideoModel === 'fal-veo3') {
        ...
        const result = await trpcClient.generation.klingVideo.mutate({
          ...
          imageUrl,
          duration:
            selectedVideoDuration === 5 || selectedVideoDuration === 10 ? selectedVideoDuration : 5,
          aspectRatio: videoRatio,
          negativePrompt: negativePrompt || undefined,
          cfgScale: 0.5,
        });
    ...
    onError: (error) => {
      ...
      if (error instanceof Error) {
        ...
        if (
          errorMessage.toLowerCase().includes('content checker') ||
...
Try switching to Veo3, Kling 2.5, or Wan 2.5 which have more flexible content policies, or ensure all characters are original creations.`;
            ...
            errorDescription =
              'The AI service detected content that violates its usage policies. Try using different characters or switching to another video model.';
          }
        } else if (
          ...
          errorDescription =
            'This request includes unlicensed characters or content. Please ensure all characters in your scene are properly licensed or use different characters.';
        } else if (
          ...
          errorDescription =
            'The character image could not be accessed. Please try regenerating the character or use a different image.';
        } else if (
          ...
          errorDescription =
            "You've reached the API usage limit. Please try again later or contact support.";
        } else if (
