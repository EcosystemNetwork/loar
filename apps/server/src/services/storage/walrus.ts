...
export class WalrusProvider implements StorageProvider {
  ...
  constructor() {
    this.publisherUrl = process.env.WALRUS_PUBLISHER_URL || DEFAULT_PUBLISHER;
    this.aggregatorUrl = process.env.WALRUS_AGGREGATOR_URL || DEFAULT_AGGREGATOR;
  }
  ...
  async upload(buffer: Buffer, filename: string, _mimeType?: string): Promise<UploadResult> {
    try {
      ...
      if (!blobId) {
        throw new Error(`Walrus upload response missing blobId: ${JSON.stringify(result)}`);
      }
  ...
  async download(blobId: string): Promise<Uint8Array> {
    ...
    if (!response.ok) {
      throw new Error(`Walrus download failed: HTTP ${response.status}`);
    }
