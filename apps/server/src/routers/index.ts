    ...
    generateFromVideo: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        try {
          ...
          throw new Error(
            `Wiki generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`
          );
        }
      }),
    ...
    improveVideoPrompt: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        try {
          ...
          throw new Error(
            `Prompt improvement failed: ${error instanceof Error ? error.message : 'Unknown error'}`
          );
        }
      }),
  }),
  ...
  synapse: router({
    ...
    download: publicProcedure.input(z.object({ pieceCid: z.string() })).query(async ({ input }) => {
      try {
        ...
        throw new Error(
          `Failed to download from Filecoin: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
      }
    }),
