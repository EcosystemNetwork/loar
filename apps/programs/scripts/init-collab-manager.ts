/**
 * One-time collab_manager initialization on devnet/mainnet.
 *
 ...
 *   COLLAB_PLATFORM                 platform key (defaults to deployer)
 *   COLLAB_PLATFORM_FEE_BPS         default: 500 (5%)
...
async function main() {
  ...
  const idlPath = resolve(__dirname, '../target/idl/collab_manager.json');
  const idl = JSON.parse(readFileSync(idlPath, 'utf-8'));
  const program = new anchor.Program(idl, provider);
  const programId = program.programId;

  const platform = new PublicKey(process.env.COLLAB_PLATFORM ?? deployerKp.publicKey.toBase58());
  const platformFeeBps = Number(process.env.COLLAB_PLATFORM_FEE_BPS ?? '500');

  const [configPda] = PublicKey.findProgramAddressSync([Buffer.from('collab_config')], programId);
  ...
  console.log(`  next_collab_id:   ${cfg.nextCollabId.toString()}`);
