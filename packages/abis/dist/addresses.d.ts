export declare const AdPlacement: {
    readonly '84532': "0x751ed220B082aE763446Fe1Fd583f3962EeBe6a3";
    readonly '11155111': "0x07c8962AB19469E48c814783664574156556B4be";
};
export type AdPlacementChainId = keyof typeof AdPlacement;
export declare const AnalyticsRegistry: {
    readonly '84532': "0xB18db49DFAB0d8B05916260D457574348893601d";
    readonly '11155111': "0x908Db578775aA2391244d57A87b156f54964aF8e";
};
export type AnalyticsRegistryChainId = keyof typeof AnalyticsRegistry;
export declare const CanonMarketplace: {
    readonly '84532': "0x222604e2185802046692293fD31dCb4Bde249Bc3";
    readonly '11155111': "0x2Bb4750B79C48e5D39c29b5e88DE05DB94A6AB1f";
};
export type CanonMarketplaceChainId = keyof typeof CanonMarketplace;
export declare const CharacterNFT: {
    readonly '84532': "0xB98A7f96827260858581Fc046c42f009adFe48D3";
    readonly '11155111': "0x286F5281114753821A18B36e5a3E532E3d19305C";
};
export type CharacterNFTChainId = keyof typeof CharacterNFT;
export declare const CollabManager: {
    readonly '84532': "0x7bB6cDdd392Bf8a6a6E58fd8600B87c8455E8240";
    readonly '11155111': "0x43883B2BE785FCC2C098401F07a17a3BbBDAf7d7";
};
export type CollabManagerChainId = keyof typeof CollabManager;
export declare const CreditManager: {
    readonly '84532': "0x6077Ce7CB99Bfe1eC3c67f8635a597a76e3FbB71";
    readonly '11155111': "0x42d5F92F550D42B36a119949d26a34Ca1F6b2d1E";
};
export type CreditManagerChainId = keyof typeof CreditManager;
export declare const EntityEditionNFT: {
    readonly '84532': "0x711eC315392f6f9FFd37e673B35acc63b9999323";
    readonly '11155111': "0x8CC865192b1A7E3A935ea50Da4dA569FC9c8e8E2";
};
export type EntityEditionNFTChainId = keyof typeof EntityEditionNFT;
export declare const EntityNFT: {
    readonly '84532': "0xD8b49c99aDb51575eea4FB795645fc9e1ce4Fa9C";
    readonly '11155111': "0x67FA9c9B19861A337E6F2222B204dBe4Ba0a98a0";
};
export type EntityNFTChainId = keyof typeof EntityNFT;
export declare const EpisodeEditionCollection: {
    readonly '84532': "0x41dcf76A5a655d25BFC00819268925a53226dBdF";
    readonly '11155111': "0x9a0c61C4221ac3CB19593941b64CA3A667b9d9dF";
};
export type EpisodeEditionCollectionChainId = keyof typeof EpisodeEditionCollection;
export declare const EpisodeNFT: {
    readonly '84532': "0xd70a0a63D1f80D6f28Beb3E8F3fC2a34dbEc3618";
    readonly '11155111': "0xB8456352f46c52461f8d70003f8D502fEE97B7C8";
};
export type EpisodeNFTChainId = keyof typeof EpisodeNFT;
export declare const IdentityNFT: {
    readonly '84532': "0x60F419E0f37dD661AF43e5326872733e89911DA9";
    readonly '11155111': "0x45A5E2816696fE9AF96e2F669cafD376a84E8db7";
};
export type IdentityNFTChainId = keyof typeof IdentityNFT;
export declare const LaunchpadStaking: {
    readonly '84532': "0xc934b9785c139d98aecE97D75a945E3f6132B49a";
};
export type LaunchpadStakingChainId = keyof typeof LaunchpadStaking;
export declare const LicensingRegistry: {
    readonly '84532': "0x8e6c09198267B07E3FC8C66F0343759111D63016";
    readonly '11155111': "0x60F419E0f37dD661AF43e5326872733e89911DA9";
};
export type LicensingRegistryChainId = keyof typeof LicensingRegistry;
export declare const LoarFaucet: {
    readonly '84532': "0x5Cc879CE26E38e4c9dAeecc0318f0EbdC22aa806";
};
export type LoarFaucetChainId = keyof typeof LoarFaucet;
export declare const LoarFeeLocker: {
    readonly '84532': "0x40e4e01735Be9e8cC5eF64E1f36188e1763e9740";
    readonly '11155111': "0x965f5C192E38b86Fa4a79A561E695C48B1DC3582";
};
export type LoarFeeLockerChainId = keyof typeof LoarFeeLocker;
export declare const LoarHookStaticFee: {
    readonly '84532': "0xe35adBBc6da1000BE4DCbf49ccBE3B9B70c9a8cC";
    readonly '11155111': "0xF5b2676E0fbc7551ae3E38f25D87C941C5a968CC";
};
export type LoarHookStaticFeeChainId = keyof typeof LoarHookStaticFee;
export declare const LoarLpLockerMultiple: {
    readonly '84532': "0xa450Bde3120a23EE3AbB87fDa4fB0E9e9F6D0307";
    readonly '11155111': "0x7d30fd57e44aB0ca407D312976816E7052905E0A";
};
export type LoarLpLockerMultipleChainId = keyof typeof LoarLpLockerMultiple;
export declare const LoarSwapRouter: {
    readonly '84532': "0x69c2aA66B3bB3e5f6658Dc2a77022558e7022398";
    readonly '11155111': "0x7E156f3Ddd56539aB941DeEfEd1342ae5C9C09a5";
};
export type LoarSwapRouterChainId = keyof typeof LoarSwapRouter;
export declare const LoarToken: {
    readonly '84532': "0x1Ff9e293D6D4D564B99CFe57fe61f4DCdac4b5D5";
    readonly '11155111': "0xAEC35cAAE68de337711E3bc06b51aaAa5551b63F";
};
export type LoarTokenChainId = keyof typeof LoarToken;
export declare const PaymentRouter: {
    readonly '84532': "0x3a6C6Bc90F34839a4792c107d9597a92fBCCA984";
    readonly '11155111': "0x0fF81B57D5B47AC5bF2A84EeA69cCf4Aa6eb0C7C";
};
export type PaymentRouterChainId = keyof typeof PaymentRouter;
export declare const PositionManager: {
    readonly '84532': "0x6D5CEf09F044224A51bd59EB841769255070e5dA";
};
export type PositionManagerChainId = keyof typeof PositionManager;
export declare const RevenueModuleFactory: {
    readonly '84532': "0xE15d941140E5504Af7C1b56Ac14da236963A99AE";
    readonly '11155111': "0x6D5CEf09F044224A51bd59EB841769255070e5dA";
};
export type RevenueModuleFactoryChainId = keyof typeof RevenueModuleFactory;
export declare const RightsRegistry: {
    readonly '84532': "0x3EF8d96cf4336E46cc7091A2325B19f53b65b109";
    readonly '11155111': "0x82b4Fe50cE07a64CbF5f97E9d70F2cEb8af63EA3";
};
export type RightsRegistryChainId = keyof typeof RightsRegistry;
export declare const SplitRouter: {
    readonly '84532': "0x8370F54A01Fc035f89293272C597bCE3B1289FC4";
};
export type SplitRouterChainId = keyof typeof SplitRouter;
export declare const StoryBounties: {
    readonly '84532': "0x43C48c9b0d2A7B408E19fB7825b924F9e1CF4901";
};
export type StoryBountiesChainId = keyof typeof StoryBounties;
export declare const SubscriptionManager: {
    readonly '84532': "0x056dDe6c068cE3FE17C2E6eE6cfA8F76eB5A5264";
    readonly '11155111': "0xdce691E3743Ad757D079E14D9C7BDf888eBFa395";
};
export type SubscriptionManagerChainId = keyof typeof SubscriptionManager;
export declare const TokenVesting: {
    readonly '84532': "0x36E25222f7E5C6f4dC8f918B68C61da83330C97F";
    readonly '11155111': "0x5d74D9e42a52D04DEC9F895F2c9D2e14b1DdCD64";
};
export type TokenVestingChainId = keyof typeof TokenVesting;
export declare const Universe: {
    readonly '11155111': "0x1427Df0998e28DB1b82C4bdBe1B503Bf9b98c76f";
};
export type UniverseChainId = keyof typeof Universe;
export declare const UniverseFactory: {
    readonly '84532': "0xbF0Fed6125b1e05aA3Dc52B72B5cd7703990627C";
    readonly '11155111': "0x8cbC5dD5aF03121F88646B9F56f414557B04dFA6";
};
export type UniverseFactoryChainId = keyof typeof UniverseFactory;
export declare const UniverseManager: {
    readonly '84532': "0xE981454B4149BEA3a9018fa2ab77482F388ba01f";
    readonly '11155111': "0x5441273a432821d20C949768d5940960dEaC6C35";
};
export type UniverseManagerChainId = keyof typeof UniverseManager;
export declare const UniverseMetadataRenderer: {
    readonly '84532': "0x43883B2BE785FCC2C098401F07a17a3BbBDAf7d7";
    readonly '11155111': "0x016b0144721e3A3134d52a32F7D712d60829d559";
};
export type UniverseMetadataRendererChainId = keyof typeof UniverseMetadataRenderer;
export declare const UniverseTokenDeployer: {
    readonly '84532': "0x0fF81B57D5B47AC5bF2A84EeA69cCf4Aa6eb0C7C";
    readonly '11155111': "0x79Cc76AF00c97CA6C45E371940ee86b2376baaFC";
};
export type UniverseTokenDeployerChainId = keyof typeof UniverseTokenDeployer;
//# sourceMappingURL=addresses.d.ts.map