...
export const UniverseProgram: ClusterMap = {
...
export const EpisodeProgram: ClusterMap = {
...
export const PaymentProgram: ClusterMap = {
...
export const LoarMint: ClusterMap = {
...
export const BubblegumTree: ClusterMap = {
 ...
 */
export function getSolanaAddress(map: ClusterMap, cluster: SolanaCluster, label: string): string {
  const value = map[cluster];
  if (!value) {
    throw new Error(
      `[solana-addresses] ${label} is not configured for cluster "${cluster}". ` +
