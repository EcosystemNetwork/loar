/**
 * Deploy "Cyber War" universe + $CYWAR token — full end-to-end.
 *
 * Flow:
 *   1. Generate cover image via Google Imagen 3
 ...
 *
 * Usage: pnpm tsx scripts/create-cyber-war.ts
 *
...
const COVER_PROMPT = [
  ...
  'A massive futuristic cyberpunk battlefield where holographic data streams collide with physical warfare.',
  'Armies of chrome-plated combat drones clash with neon-armored hackers riding digital waveforms.',
  'A towering sentient AI manifests as a godlike figure made of circuit boards and liquid code above a burning megacity.',
  'A lone female hacker in a glowing visor crouches on a shattered server tower, fingers trailing streams of electric-blue data.',
  'Deep cyan, hot magenta, toxic green, and black color palette. Volumetric neon fog, particle effects, glitch artifacts.',
  'Ultra-detailed 8K cinematic concept art, dramatic wide angle, cyberpunk aesthetic, no text, no watermarks, no logos.',
].join(' ');
...
// ── Step 1: Generate cover image via Google Imagen 3 ─────────────────────────
...
async function main() {
  console.log('\n' + '═'.repeat(60));
  console.log('  LOAR — Cyber War Universe + $CYWAR Token Deployment');
  ...
  // Step 2: Pin to IPFS
  const imageUrl = await pinToPinata(imageBuffer, `cywar-cover.jpg`);
