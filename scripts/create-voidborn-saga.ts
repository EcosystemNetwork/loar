/**
 * Deploy "Voidborn Saga" universe + $VOID token — full end-to-end.
 *
 * Flow:
 ...
 *
 * Usage: pnpm tsx scripts/create-voidborn-saga.ts
 *
...
const UNIVERSE_DESCRIPTION =
  'Five aliens crash-land a beat-up ship called the Starling in a wooded ravine outside a California suburb and have to survive a single night on Earth with no repair parts, no phones, and one dangerously outdated briefing. Zix, the self-appointed captain, wants to remain hidden, repair the ship, and leave the planet undisturbed. Mora, the engineer, is too busy keeping everything from catching fire to argue. Pebb immediately pledges allegiance to Taco Bell. Drael falls in love with Earth nightlife the moment he sees a human woman laugh. Nuni, the anthropologist who confidently classified humans as "probably extinct," is revising her research at speed. On the first night alone they survive a hiker, a convenience store clerk, two meteor-hunting teens, and a distorted radio voice from a long-buried Voidborn sleeper network warning them about a man named Hector — and about the planet itself. "Earth changes you," the voice says. "That is your first warning." By dawn the mission is clear: contact the sleepers at the old observatory, fix the ship, and get off this planet immediately. By dawn, nobody actually wants to leave. Voidborn Saga is a character-driven animated comedy about being spectacularly out of your depth in suburban America, and about what it means when the people you were sent to study are more complicated, more welcoming, and more dangerous than anyone on your homeworld warned you.';

const COVER_PROMPT = [
  `Epic animated series cover art for "${UNIVERSE_NAME}".`,
  'Five small alien figures in silhouette stand on a bald California hilltop at pre-dawn, backs to camera,',
  'looking out over a sprawling Southern California suburb glowing with neon: tract homes, palm trees, a glowing strip mall with Taco Bell and convenience-store signs,',
  'a freeway of headlight streaks, and on a far hill an old observatory dome catching the first blue-gold light of sunrise.',
  'Behind and above them, a junky beloved alien ship crashed into a wooded ravine, wisps of smoke rising into a fading star-filled sky.',
  'The five silhouettes: a tall lanky figure with a dramatic sash, a compact wiry figure with a tool harness, a tiny rotund figure with glowing fangs hugging a bag of chips,',
  'a broad-shouldered hero silhouette with faintly glowing eyes, and a slim nervous figure holding a cracked tablet.',
  'A helicopter blinks red across the sky. A taco wrapper blows past their feet. Deep in the city, a tiny blue-green signal blinks back.',
  'Pixar-quality animated feature style, warm neon contrast against cool dawn, cinematic 2.39:1 widescreen composition, no text, no watermarks, no logos.',
].join(' ');
...
async function registerInFirestore(deploy: DeployResult, imageUrl: string): Promise<void> {
  ...
  // 5. Sign creation message (matches UI format)
...
async function main() {
  console.log('\n' + '═'.repeat(60));
  console.log('  LOAR — Voidborn Saga Universe + $VOID Token Deployment');
  ...
  // Step 2: Pin to IPFS
  const imageUrl = await pinToPinata(imageBuffer, `voidborn-cover.jpg`);
