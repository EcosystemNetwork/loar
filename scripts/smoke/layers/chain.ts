...
// Not every chain in CHAINS has a recorded deployment (e.g. Base Sepolia) —
// skip missing entries instead of crashing the whole hermetic smoke suite at
// import time; runChainLayer below already falls back to sepolia's address
// when a chain-specific one isn't present.
const UNIVERSE_MANAGER_ADDRESSES: Partial<Record<number, `0x${string}`>> = {};
for (const id of [sepolia.id, baseSepolia.id]) {
  const raw = (UniverseManagerAddresses as any)[String(id)];
  if (raw) UNIVERSE_MANAGER_ADDRESSES[id] = getAddress(raw);
}
...
export async function runChainLayer(cfg: SmokeConfig): Promise<ChainResult> {
  ...
  if (!UNIVERSE_MANAGER_ADDRESS) {
    throw new Error(
      `No UniverseManager address recorded for chain ${cfg.chainId} or the sepolia fallback — check packages/abis/src/addresses.ts`
    );
  }
