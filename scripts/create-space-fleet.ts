/**
 * Deploy "Space Fleet" universe + $SF token — full end-to-end.
 *
 * Flow:
 ...
 *
 * Usage: pnpm tsx scripts/create-space-fleet.ts
 *
...
// ── Universe config ───────────────────────────────────────────────────────────
const UNIVERSE_NAME = 'Space Fleet';
const TOKEN_SYMBOL = 'SF';
const UNIVERSE_DESCRIPTION =
  "The United Earth Government proudly presents its space program: three orbital stations, a lunar research base, and plans for a Mars colony by 2145. What the public doesn't know is that behind this modest facade lies humanity's true reach — a massive interstellar armada spanning twelve star systems, with over 40,000 warships, deep-space colonies housing billions, and first-contact protocols already in use with seven alien civilizations. The \"Space Fleet\" is not a dream for the future — it's been operational for decades. When a junior analyst at the public space agency stumbles onto classified transmissions from a battle group fighting an existential war near Proxima Centauri, the greatest cover-up in human history begins to unravel. The question isn't whether we're alone in the universe — it's why our own government has been lying about winning a war among the stars.";

const COVER_PROMPT = [
  `Epic cinematic movie poster for "${UNIVERSE_NAME}".`,
  'A split-reality composition: the bottom half shows a modest, sterile government space agency press room with a small rocket model on display,',
  'while the top half reveals the true scale — an impossibly vast armada of thousands of massive warships emerging from hyperspace above an alien world.',
  'A lone analyst in a government suit stares up through a cracking ceiling that dissolves into the star-filled void, seeing the truth for the first time.',
  'Massive dreadnoughts with running lights stretch to the horizon, dwarfing planets.',
  'Contrast of mundane bureaucratic beige below with epic deep-space navy blue, silver, and burning orange engine trails above.',
  'Ultra-detailed 8K cinematic concept art, dramatic vertical split composition, military sci-fi aesthetic, no text, no watermarks, no logos.',
].join(' ');
...
async function main() {
  console.log('\n' + '═'.repeat(60));
  console.log('  LOAR — Space Fleet Universe + $SF Token Deployment');
  ...
  // Step 2: Pin to IPFS
  const imageUrl = await pinToPinata(imageBuffer, `sf-cover.jpg`);
