/**
 * Repair Monerochan media URLs pointing at the dead private Pinata gateway
 ...
 * restriction). The underlying CIDs are still live on the public network —
 * verified via HEAD against gateway.pinata.cloud before this script was
 * written. Rewrites every affected field to the public gateway equivalent
 * and archives the old URL alongside it for reversibility.
 *
 * Scope: episodes.clips[].videoUrl, content.mediaUrl/videoUrl,
 *        cinematicUniverses.image_url, entities.imageUrl — all scoped to
 *        the Monerochan universe (0x0000...19d9e1c8a49).
 *
 * Usage:
 *   DRY_RUN=1 pnpm tsx scripts/fix-monerochan-dead-gateway-urls.ts   # preview
 *   pnpm tsx scripts/fix-monerochan-dead-gateway-urls.ts             # apply
 *
 * Env:
...
async function main() {
  ...
  const app = initializeApp({ credential: cert(sa) }, `mc-gw-fix-${Date.now()}`);
  const db: Firestore = getFirestore(app);
  db.settings({ preferRest: true });

  console.log(`\n=== Monerochan dead-gateway URL repair (${DRY_RUN ? 'DRY-RUN' : 'LIVE'}) ===\n`);
  ...
  for (const doc of contentSnap.docs) {
    ...
    for (const field of ['mediaUrl', 'videoUrl']) {
      const v = x[field];
  ...
  const backupPath = path.resolve(
    process.cwd(),
    `repair-monerochan-gateway-urls-${Date.now()}.json`
  );
